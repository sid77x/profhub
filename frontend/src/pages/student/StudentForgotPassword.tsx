...
const StudentForgotPassword: React.FC = () => {
  ...
  const handleRequestOtp = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await axios.post(`${API_URL}/students/forgot-password/request-otp`, { email });
      setStage('otp');
  ...
  const handleVerifyOtpAndReset = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await axios.post(`${API_URL}/students/forgot-password/reset`, {
        email,
        otp,
    ...
    <div className="min-h-screen bg-background flex flex-col justify-center py-12 sm:px-6 lg:px-8 relative overflow-hidden">
      ...
      >
        <button
          ...
          className="flex items-center gap-2 text-secondary hover:opacity-80 transition-colors mb-4"
        >
