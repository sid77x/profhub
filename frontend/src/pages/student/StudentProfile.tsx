...
const StudentProfile: React.FC = () => {
  ...
  const [formData, setFormData] = useState({ name: '', email: '', reg_no: '', department: '', year: '', college_name: '', bio: '', skills: [] as string[], resume_url: '' });
  const [skillInput, setSkillInput] = useState('');

  useEffect(() => { if (studentId) fetchStudent(studentId); }, [studentId, fetchStudent]);
  useEffect(() => {
    if (student) setFormData({ name: student.name || '', email: student.email || '', reg_no: student.reg_no || '', department: student.department || '', year: String(student.year || ''), college_name: student.college_name || '', bio: student.bio || '', skills: student.skills || [], resume_url: student.resume_url || '' });
  }, [student]);
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-6">
            {[
              ...
              <div key={field.name}>
                ...
                <input type="text" name={field.name} value={(formData as any)[field.name]} onChange={handleInputChange}
                  disabled={!isEditing || !field.editable} className={inputClass} required={field.required} />
              </div>
            ))}
          </div>
